...
<div
  class="flex flex-col lg:flex-row w-full min-h-screen bg-gradient-to-b from-[#0f172a] to-[#111827] pt-20 px-4 lg:px-20 gap-10 overflow-x-hidden"
>
  <!-- KALENDARZ -->
  <div class="w-full lg:w-[800px]">
    <div class="text-center text-pink-400 mb-6">
      <h2
        class="text-4xl font-extrabold bg-gradient-to-r from-purple-400 to-blue-400 text-transparent bg-clip-text"
      >
        {months[currentMonth]}
        {currentYear}
      </h2>
    ...
    <div class="flex justify-between mb-6">
      <button
        class="px-5 py-3 bg-gray-700 text-white rounded-md hover:bg-gray-600 transition"
    ...
    <div
      class="grid grid-cols-7 gap-4 bg-[#1e293b] border border-pink-500 rounded-lg p-6 shadow-lg"
    >
      {#each dates as { day, weekday, events, date }}
        ...
        >
          ...
          {#if events.length > 0}
            <div
              class="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-pink-500"
            ></div>
          {/if}
        </div>
      {/each}
  ...
  <!-- PRAWA SEKCJA -->
  <div
    class="w-full lg:w-[800px] mt-4 lg:mt-20 grid grid-cols-1 md:grid-cols-2 gap-6 self-start ml-auto"
  >
    <!-- SZCZEGÓŁY -->
    <div
    ...
    >
      ...
      {#if hoveredEvents.length > 0}
          ...
          {#each hoveredEvents as event}
            <li class="text-gray-300 mb-4">
              ...
              <span class="text-sm text-pink-300">
                ...
                {formatPolishDateTime(event.start_date)} - {formatPolishDateTime(
                  event.end_date
                )}
              </span>
            </li>
        ...
        {#if hoveredEvents[0].video_link}
          <button
            class="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors"
            on:click={() => handleWatchButtonClick(hoveredEvents[0])}
          >
            Watch
          </button>
        {/if}
    ...
    <!-- MOJE -->
    <div
      class="p-6 bg-[#1e293b] border border-pink-500 rounded-lg shadow-lg min-h-[300px]"
    >
...
{#if showPopup}
  ...
  >
    <div
      ...
      style="top: {popupPosition.y}px; left: {popupPosition.x}px; width: {isMinimized
        ? '20%'
        : '80%'}; height: {isMinimized ? '20%' : '80%'};"
    ...
    >
      ...
      <div class="absolute top-2 right-2 flex gap-2">
        <button
          ...
          on:click={closePopup}>X</button
        >
        <button
          class="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600"
          on:click={minimizePopup}>_</button
        >
      </div>
